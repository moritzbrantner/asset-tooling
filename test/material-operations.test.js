...
test("material operation registry exposes ORM packing and PBR bundle assembly", () => {
  ...
  assert.deepEqual(
    MATERIAL_OPERATIONS.find((operation) => operation.id === "texture.orm.pack").inputs.map((input) => input.id),
    ["ambient-occlusion", "roughness", "metallic"],
  );
  assert.deepEqual(
    MATERIAL_OPERATIONS.find((operation) => operation.id === "material.pbr.bundle").inputs.map((input) => input.id),
...
test("ORM packing maps grayscale AO roughness metallic to linear RGB with opaque alpha", async () => {
  ...
  const result = await executeTextureOrmPackOperation(root, {
    inputs: { "ambient-occlusion": ambientOcclusion, roughness, metallic },
    parameters: {},
  });
  ...
  assert.deepEqual(result.outputs.output.metadata.inputs.map((input) => input.port), [
    "ambient-occlusion",
    "roughness",
    "metallic",
  ]);
...
test("ORM packing rejects mismatched dimensions before output identity is accepted", async () => {
  ...
  await assert.rejects(
    () =>
      executeTextureOrmPackOperation(root, {
        inputs: { "ambient-occlusion": ambientOcclusion, roughness, metallic },
      }),
...
test("PBR material bundle stores canonical typed references and is idempotent", async () => {
  ...
  const orm = (
    await executeTextureOrmPackOperation(root, {
      inputs: { "ambient-occlusion": channel, roughness: channel, metallic: channel },
    })
  ).outputs.output;

  const invocation = {
    inputs: { "base-color": baseColor, normal, orm },
  ...
  assert.deepEqual(first.outputs.output.metadata.texturePorts, ["base-color", "normal", "orm"]);
});
