...
The strict project now covers the package/public-wiring authority, deterministic identity foundation, core asset-operation contract, generation/backend normalization contract, and content-addressed storage/cache boundaries:
...
Later typing slices should continue expanding outward from these stable primitives into cohesive boundaries rather than adding isolated files solely to increase compiler coverage. The asset-spec/schema document is the next natural boundary: it is the runtime-validation authority that feeds backend selection, cache paths, declared artifacts, and eventually `src/core.ts`. Typing it before concrete backends and orchestration prevents those layers from inventing competing representations of the validated specification.
