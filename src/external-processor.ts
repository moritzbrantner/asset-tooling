...
export function normalizeExternalProcessor(
  value: unknown,
  operationId: string,
): ExternalProcessor {
  ...
  for (const key of Object.keys(processor)) {
    if (!allowed.has(key)) {
      throw new Error(`${operationId} processor contains unknown field '${key}'`);
    }
  }
  ...
  if (!Array.isArray(prefixArguments)) {
    throw new Error("processor.prefixArguments must be an array");
  }
...
function processorStorageEnvironment(processor: ExternalProcessor): Record<string, string> {
...
async function gitOutput(
  ...
  try {
    ...
    throw new Error(`${location} failed: ${(error as Error).message}`);
  }
}

async function verifyProcessorSource(
  ...
  if (processor.checkoutRoot !== undefined) {
    ...
    if (unexpectedStatus.length > 0) {
      throw new Error(
        `${operationId} processor checkout must be source-clean at the declared revision`,
      );
    }
  ...
  for (const sourceFile of sourceFiles) {
    ...
    try {
      ...
      throw new Error(
        `${operationId} processor source file '${sourceFile}' could not be read: ${(error as Error).message}`,
      );
    }
...
function normalizeProbe(
  ...
  const matching = components.filter(
    (component) => isPlainObject(component) && component.id === options.processorId,
  );
  if (matching.length !== 1) {
    throw new Error(
      `processor probe must contain exactly one '${options.processorId}' component`,
    );
  }
  const component = assertPlainObject(
    matching[0],
    `${options.operationId} processor probe component`,
  );
  ...
  if (component.protocol !== options.protocol) {
    throw new Error(`processor probe protocol must be '${options.protocol}'`);
  }
  if (component.codec !== options.codec) {
    throw new Error(`processor probe codec must be '${options.codec}'`);
  }
...
export async function resolveExternalProcessorIdentity(
  root: string,
  processorValue: unknown,
  options: ResolveOptions,
) {
  if (typeof root !== "string" || !path.isAbsolute(root)) {
    throw new Error(`${options.operationId} root must be an absolute path`);
  }
  const processor = normalizeExternalProcessor(processorValue, options.operationId);
  const source = await verifyProcessorSource(processor, root, options.operationId);
  ...
  const probe = normalizeProbe(components as unknown[], options);
  return {
    processor,
    environment,
    implementation: {
