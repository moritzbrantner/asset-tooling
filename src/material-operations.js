...
async function createBuildIdentity(root, operation, parameters, inputs) {
  ...
  if (operation.id === "texture.orm.pack") {
    const images = await Promise.all(
      ORM_PORTS.map((id) => readSrgb(assetRoot, build.inputs[id])),
    );
...
export async function executePbrMaterialBundleOperation(root, invocation = {}) {
  ...
  for (const [port, asset] of Object.entries(build.inputs)) {
    const image = port === "orm" ? await readLinear(assetRoot, asset) : await readSrgb(assetRoot, asset);
    textures[MATERIAL_TEXTURE_KEYS[port]] = textureEntry(asset, image);
  }
  const document = {
    schemaVersion: 1,
