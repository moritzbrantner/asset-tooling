...
export const LINEAR_RGBA8_IMAGE_MEDIA_TYPE =
  "application/vnd.moritzbrantner.linear-rgba8+json";
export const LINEAR_RGBA8_IMAGE_SCHEMA_VERSION = 1;
...
export function createLinearRgba8Image({ width, height, pixels }) {
  ...
  if (!(pixels instanceof Uint8Array)) {
    throw new Error("linear RGBA8 image pixels must be a Uint8Array");
  }
  ...
  if (pixelBytes.length !== expected) {
    throw new Error(`linear RGBA8 image pixels must contain exactly ${expected} bytes`);
  }
  return {
    schemaVersion: LINEAR_RGBA8_IMAGE_SCHEMA_VERSION,
...
export function parseLinearRgba8Image(bytes) {
  ...
  if (document.schemaVersion !== LINEAR_RGBA8_IMAGE_SCHEMA_VERSION) {
    throw new Error(
      `linear RGBA8 image schemaVersion must be ${LINEAR_RGBA8_IMAGE_SCHEMA_VERSION}`,
    );
  }
  const width = dimension(document.width, "linear RGBA8 image width");
  const height = dimension(document.height, "linear RGBA8 image height");
  if (document.colorSpace !== "linear-srgb") {
    throw new Error("linear RGBA8 image colorSpace must be 'linear-srgb'");
  }
  ...
  const pixels = canonicalBase64(
    ...
    "linear RGBA8 image pixelsBase64",
  );
  const expected = expectedByteLength(width, height);
  if (pixels.length !== expected) {
    throw new Error(
      `linear RGBA8 image pixelsBase64 must decode to exactly ${expected} bytes`,
    );
  }
  return { width, height, colorSpace: "linear-srgb", alphaMode: "straight", pixels };
}
